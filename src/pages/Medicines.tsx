...
const Medicines: React.FC = () => {
    ...
    const [repetition, setRepetition] = useState<'nao' | 'diario' | 'semanal' | 'mensal'>('nao');
    ...
    const handleAddOrUpdate = (e: React.FormEvent) => {
        ...
        const med: Medicine = {
            ...
            dosage,
            repetition,
            repetitionInterval: repetition === 'diario' ? Number(repetitionInterval) : undefined,
            durationType,
    ...
    const handleEdit = (id: string) => {
        ...
        if (med) {
            ...
            setRepetitionInterval(med.repetitionInterval ? med.repetitionInterval.toString() : '1');
    ...
    return (
        <Box sx={{ width: '100vw', minHeight: '100vh', bgcolor: '#f4f8fc', py: 6 }}>
            <Paper elevation={6} sx={{ maxWidth: 600, mx: 'auto', p: { xs: 2, sm: 4 }, borderRadius: 4 }} role="main" aria-label="Cadastro e lista de medicamentos">
                <h2 style={{ fontSize: '2.5rem', marginBottom: 32, textAlign: 'center', color: '#1976d2', letterSpacing: 1, fontWeight: 900, textShadow: '1px 1px 0 #fff' }}>
                ...
                <Select
                    value={repetition}
                    onChange={e => setRepetition(e.target.value as 'nao' | 'diario' | 'semanal' | 'mensal')}
                    fullWidth
                    displayEmpty
                    style={{ marginBottom: 8 }}
                >
                ...
                <Select
                    value={durationType}
                    onChange={e => setDurationType(e.target.value as 'sempre' | 'quantidade' | 'data')}
                    fullWidth
                    style={{ marginBottom: 8 }}
                >
                ...
                <label style={{ fontWeight: 'bold', marginBottom: 2, fontSize: '1.3rem' }}>Horário(s) para tomar</label>
                {times.map((time, idx) => (
                    <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 6 }}>
                        ...
                        />
                        {times.length > 1 && (
                            <Button type="button" onClick={() => setTimes(times.filter((_, i) => i !== idx))} color="error" variant="outlined" style={{ minWidth: 40 }} aria-label="Remover horário">➖</Button>
                        )}
                        {idx === times.length - 1 && (
                            <Button type="button" onClick={() => setTimes([...times, ''])} color="primary" variant="outlined" style={{ minWidth: 40 }} aria-label="Adicionar novo horário">+</Button>
                        )}
                    </div>
                ))}
                <Button
                    ...
                    fullWidth
                    style={{ fontWeight: 'bold', fontSize: '1.2rem', marginTop: 12 }}
                    aria-label={editId !== null ? 'Atualizar medicamento' : 'Adicionar medicamento'}
                >
                    {editId !== null ? 'Atualizar' : 'Adicionar'}
                </Button>
                </Box>
            <ul style={{ marginTop: 36, padding: 0, listStyle: 'none', width: '100%' }} aria-label="Lista de medicamentos cadastrados">
                {medicines.map(med => (
                    <li
                        key={med.id}
                        style={{
                    ...
                    >
                        <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 12 }}>
                            <span role="img" aria-label="Remédio" style={{ fontSize: '2rem' }}>💊</span>
                            <b style={{ fontSize: '1.5rem', color: '#003366', fontWeight: 900 }}>{med.name}</b>
                        </div>
                        <div style={{ fontSize: '1.1rem', color: '#222', marginTop: 2, lineHeight: 2 }}>
                            {med.description && <><b>Descrição:</b> {med.description} <br /></>}
                            {/* Dosagem */}
                            {med.dosage && <><b>Dosagem:</b> {med.dosage} <br /></>}
                            {/* Repetição */}
                            <b>Repetição:</b> {med.repetition === 'nao' ? 'Não repetir' : med.repetition === 'diario' ? `Diariamente${med.repetitionInterval ? ` (a cada ${med.repetitionInterval} dias)` : ''}` : med.repetition === 'semanal' ? 'Semanalmente' : 'Mensalmente'} <br />
                            {/* Duração */}
                            {med.durationType === 'sempre' && <><b>Duração:</b> Sempre <br /></>}
                            {med.durationType === 'quantidade' && med.durationAmount && (
                                <><b>Duração:</b> {med.durationAmount} vezes <br /></>
                            )}
                            {med.durationType === 'data' && med.durationEndDate && (
                                <><b>Duração até:</b> {new Date(med.durationEndDate).toLocaleDateString()} <br /></>
                            )}
                            {med.times && med.times.length > 0 && (
                                <><b>Horário(s):</b> {med.times.join(', ')} <br /></>
                            )}
                            {/* id_user removido */}
                        </div>
                        <div style={{ display: 'flex', gap: 18, marginTop: 10 }}>
                            ...
                            >
                                <span role="img" aria-label="Editar"></span> Editar
                            </button>
                            <button
                            ...
                            >
                                <span role="img" aria-label="Excluir"></span> Excluir
                            </button>
                        </div>
                    </li>
                ))}
